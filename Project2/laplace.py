...
h1error = gridFunction(dot(grad(uh - exact), grad(uh - exact)), uh.space.gridView, name="h1error", order=uh.space.order)
errorGlobal = []
dofsGlobal  = []
for count in range(3):
    laplace.solve(target=uh)
    error = numpy.sqrt(integrate(h1error, uh.space.gridView, order=5))
    errorGlobal += [error]
